...
void huffman::createMinHeap() {
    ...
    while (!inFile.eof()) {
        arr[id]->freq++;
        inFile.get(id);  //extracting character by character 
    }
    inFile.close();
...
void huffman::saveEncodedFile() {
    ...
    //displaying the size of compressed file
    outFile.open(outFileName, ios::in | ios::binary);
    outFile.seekg(0, std::ios::end); // Move file pointer to end of file
    int Dec_File_size = outFile.tellg(); // Get current position of file pointer
    std::cout << "Decompressed File size: " << Dec_File_size << " bytes\n" << std::endl;
    outFile.close();
}
...
//displaying the size of compressed file
    outFile.open(outFileName, ios::in | ios::binary);
    outFile.seekg(0, std::ios::end); // Move file pointer to end of file
    int Org_File_size = outFile.tellg(); // Get current position of file pointer
    std::cout << "Decompressed File size: " << Org_File_size << " bytes\n" << std::endl;
    outFile.close();
}
...
void huffman::compress() {
    ...
    //displaying the size of compressed file
    inFile.open(inFileName, ios::in | ios::binary);
    inFile.seekg(0, std::ios::end); // Move file pointer to end of file
    int Org_File_size = inFile.tellg(); // Get current position of file pointer
    std::cout << "Original File size: " << Org_File_size << " bytes\n" << std::endl;
    inFile.close();
...
void huffman::decompress() {
    ...
    inFile.open(inFileName, ios::in | ios::binary);
    inFile.seekg(0, std::ios::end); // Move file pointer to end of file
    int Dec_File_size = inFile.tellg(); // Get current position of file pointer
    std::cout << "Compressed File size: " << Dec_File_size << " bytes\n" << std::endl;
    inFile.close();
